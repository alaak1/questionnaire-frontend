...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, ButtonComponent, InputComponent, CardComponent, NgIf, NgFor],
  template: `
    <h2 class="mb-4 text-xl font-semibold text-slate-800">Edit Questionnaire</h2>
    <form *ngIf="loaded" [formGroup]="form" class="flex flex-col space-y-5" (ngSubmit)="save()">
      <app-card>
...
export class QuestionnaireEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.qs.fetchOne(this.id).subscribe((q) => {
      this.form.patchValue({ title: q.title, description: q.description });
      (q.questions || []).forEach((qq) =>
        this.questions.push(
          this.fb.group({
            question_text: [qq.question_text, Validators.required],
  ...
  save() {
    ...
    this.qs.update(this.id, payload).subscribe(() => {
